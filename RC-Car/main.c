...
void get_lora_package() {
    // reserved later car func

    unsigned char pack_size = SPI_read();
    for (unsigned char i = 0; i < HEADERSIZE; i++)
        spi_buffer[!spi_buffer_index][i] = SPI_read();

    spi_buffer_index = !spi_buffer_index;

    for (unsigned char i = 0; i < pack_size; i++)
        exec_command(SPI_read());
}
...
void init() {
    ...
    for (unsigned char i = 0; i < schedule_max; i++)
        scheduler[i] = default_schedule;
